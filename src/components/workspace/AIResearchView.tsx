...
export const AIResearchView: React.FC = () => {
  ...
  const [recentSessions, setRecentSessions] = useState([
    { id: 's1', title: '沪深300低波动动量策略筛选', date: '10分钟前', active: true },
    { id: 's2', title: `标的 [${selectedStockSymbol}] 筹码与多因子诊断`, date: '1小时前', active: false },
    { id: 's3', title: '英伟达 (NVDA) Q4 财报算力需求拆解', date: '昨天', active: false },
    { id: 's4', title: '贵州茅台与宁德时代质量因子对比', date: '2天前', active: false },
    { id: 's5', title: '红利低波与科技成长股轮动逻辑', date: '3天前', active: false },
  ]);
  ...
  // Apple-styled Financial Prompts Cards
  const applePrompts = [
    {
      id: 'p1',
      tag: '行情诊股',
      title: `诊断 [${selectedStockSymbol}] 筹码与动量`,
      desc: `评估 ${selectedStockSymbol} 60日动量评分、估值分位数、筹码集中度与同业比较优势。`,
      query: `详细诊断标的 [${selectedStockSymbol}] 的 60 日动量评分、估值分位数、筹码集中度与同业比较优势。`,
      icon: TrendingUp,
    },
    {
      id: 'p2',
      tag: '因子选股',
      title: '沪深300高动量低波动精选',
      desc: '筛选近60日动量前20%、20日波动率低、换手率改善的优质标的。',
    ...
    },
    {
      id: 'p3',
      tag: '财报拆解',
      title: 'NVIDIA (NVDA) 算力财报深度剖析',
      desc: '拆解 NVDA 最新财报，重点分析 Data Center 算力需求与 Blackwell 毛利率。',
      query: '拆解 NVIDIA 最新季度财报，重点分析 Data Center 算力需求与 Blackwell 架构芯片毛利率变化趋势。',
      icon: FileText,
    },
    {
      id: 'p4',
      tag: '策略构建',
      title: '质量成长多因子策略与回测',
      desc: '以 ROE + 自由现金流为核心，设计 A 股质量成长多因子调仓模型。',
      query: '请为我设计一个以 ROE + 自由现金流为核心的 A 股质量成长多因子策略，包含因子权重、调仓周期与止损建议。',
      icon: Workflow,
    },
    {
      id: 'p5',
      tag: '宏观轮动',
      title: '红利低波与科技成长股轮动',
      desc: '评估当前利率与流动性下，高股息红利与半导体科技的调仓性价比。',
      query: '结合当前宏观利率环境与市场流动性，深度评估高股息红利股与半导体科技股的轮动性价比与调仓时机。',
      icon: RefreshCw,
    },
    {
      id: 'p6',
      ...
      desc: '基于 14 个基本面与高频特征，预测下个周期全市场 TOP10 超额股票。',
      query: '使用 LightGBM 模型基于 14 个基本面与高频因子，预测下一个 20 日调仓周期的全市场超额收益 TOP10 股票。',
      icon: Bot,
    },
  ];

  // Quick Inline Shortcut Chips
  const quickChips = [
    { label: '⚡ 策略代码生成', query: '请为我生成一套标准 Python / Backtrader 双均线与 RSI 量化策略代码范例。' },
    { label: `🔍 深度诊断 ${selectedStockSymbol}`, query: `深度分析 [${selectedStockSymbol}] 当前 K 线形态、主力资金净流入与换手率。` },
  ...
  const handleNewSession = () => {
    ...
    setRecentSessions((prev) => [
      { id: newId, title: '新量化研究会话', date: '刚刚', active: true },
    ...
    setMessages([
      {
        id: 'welcome-new',
        sender: 'assistant',
        content: '已建立全新量化会话。请输入你想研判的股票、策略或选股条件：',
        steps: ['初始化多因子引擎', '已连接 AKShare 实时数据'],
      },
    ]);
  };
  ...
  return (
    <div className="min-h-[calc(100vh-60px)] bg-slate-50/60 text-slate-800 flex flex-col font-sans selection:bg-slate-900 selection:text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Minimalist Collapsible History Sidebar */}
        {showSidebar && activeTab === 'chat' && (
          <aside className="w-64 border-r border-slate-200/60 bg-white/60 backdrop-blur-md p-4 flex flex-col justify-between shrink-0 transition-all hidden md:flex">
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs font-semibold text-slate-400 px-1">
                ...
                <span className="font-mono text-[10px]">{recentSessions.length} 条</span>
              </div>
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-mono shrink-0 ${
                        session.active ? 'text-slate-300' : 'text-slate-400 group-hover:text-slate-500'
                      }`}
                    >
                      {session.date}
                    </span>
            ...
            {/* Bottom Current Focus Target Pill */}
            <div className="p-3 bg-slate-100/70 rounded-2xl border border-slate-200/60 space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold text-slate-500">当前焦点标的</span>
                <span className="font-mono font-bold bg-white text-slate-900 px-2 py-0.5 rounded text-[10px] border border-slate-200">
                  {selectedStockSymbol}
                </span>
              </div>
              <button
                onClick={() => handleSend(`深度评估标的 [${selectedStockSymbol}] 60日动量与估值`)}
                className="w-full py-1.5 bg-white hover:bg-slate-50 text-slate-800 text-[11px] font-semibold rounded-xl border border-slate-200/80 transition-colors flex items-center justify-center gap-1"
              >
        ...
        {activeTab === 'chat' && (
          <main className="flex-1 flex flex-col justify-between overflow-y-auto p-4 md:p-8 max-w-5xl mx-auto w-full space-y-6">
            {/* Ambient Apple Hero Header when starting conversation */}
            {messages.length <= 1 && (
              <div className="my-auto py-8 space-y-8 animate-in fade-in duration-500">
                <div className="text-center space-y-3">
                  <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900">
                    探索 AI 驱动的量化研究
                  </h1>
                  <p className="text-xs md:text-sm text-slate-500 max-w-lg mx-auto font-normal">
                    基于 DeepSeek 多因子模型，提供全市场行情诊股、财报拆解、Alpha 选股与策略生成
                  </p>
                </div>

                {/* Apple-styled Clean Prompts Grid */}
                  ...
                  {applePrompts.map((p) => {
                    ...
                    return (
                      <div
                        ...
                        className="p-4 bg-white hover:bg-slate-50/80 border border-slate-200/80 hover:border-slate-300 rounded-2xl shadow-xs hover:shadow-md transition-all cursor-pointer group flex flex-col justify-between space-y-3"
                      >
                        <div className="flex items-center justify-between">
            ...
            <div className="space-y-6 flex-1">
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[88%] p-4 md:p-5 rounded-2xl text-xs md:text-sm leading-relaxed ${
                      msg.sender === 'user'
                  ...
                  >
                    ...
                    {msg.steps && msg.steps.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-100 space-y-1 font-mono text-[11px] text-slate-400">
              ...
              {loading && (
                <div className="flex items-center gap-2 text-xs text-slate-500 p-3.5 bg-white rounded-2xl w-fit border border-slate-200/70 shadow-xs animate-pulse">
                  ...
                  <span>Aether AI 正在计算中...</span>
                </div>
              )}
              <div ref={messagesEndRef} />
            </div>

            {/* Apple Floating Command Input Bar */}
            <div className="sticky bottom-4 z-20 space-y-2.5 pt-2">
              {/* Shortcut pills */}
              <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none text-[11px]">
                <button
                  type="button"
                  onClick={() => setShowUploadModal(true)}
                  className="px-3 py-1 bg-white hover:bg-slate-100 text-slate-700 rounded-full border border-slate-200/80 shadow-2xs shrink-0 transition-all flex items-center gap-1 font-medium"
                >
                ...
                {quickChips.map((chip, idx) => (
                  <button
                    ...
                    className="px-3 py-1 bg-white/80 hover:bg-white text-slate-600 hover:text-slate-900 rounded-full border border-slate-200/70 shadow-2xs shrink-0 transition-all font-medium"
                  >
                    {chip.label}
                  </button>
                ))}
              ...
              {/* Input Form Floating Capsule */}
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleSend();
                }}
                className="bg-white/90 backdrop-blur-xl rounded-3xl border border-slate-200/80 shadow-xl shadow-slate-200/40 p-3 space-y-2 focus-within:ring-2 focus-within:ring-slate-300 transition-all"
              >
                <textarea
                  rows={2}
                  value={inputPrompt}
                ...
                <div className="flex items-center justify-between px-1">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200/80 text-slate-700 text-[10px] font-mono font-bold rounded-full transition-colors flex items-center gap-1"
                  >
        ...
        {/* Document Research Tab (Clean Layout) */}
        {activeTab === 'docs' && (
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full border border-slate-200 shadow-2xl space-y-4">
            ...
            <div className="flex justify-end">
              <button
                onClick={() => setShowUploadModal(false)}
                className="px-4 py-2 bg-slate-100 text-slate-700 text-xs font-semibold rounded-full hover:bg-slate-200 transition-colors"
              >
                取消
              </button>
            </div>
